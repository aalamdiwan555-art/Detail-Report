...
only the configured build-time password hash and is available after tapping the
...
For GitHub Actions builds, configure the repository secret named
`ULTRA_ADMIN_PASSWORD_HASH`. Replit secrets are not automatically passed to
GitHub Actions.
...
and enter the configured administrator password.
